...
function CtaButton({
  ...
  const v = variant || cta.variant || "primary";
  return (
    <Button href={cta.href} variant={v as "primary"} style={styleProp}>
      {cta.label}
  ...
  return (
    ...
    >
      ...
      {(item.cta || item.secondaryCta) && (
        ...
        >
          {item.cta && <CtaButton cta={item.cta} variant="primary" />}
          {item.secondaryCta && <CtaButton cta={item.secondaryCta} variant="ghost" />}
        </div>
      )}
    </div>
  );
...
function StandardBody({ item, theme }: { item: ContentItem; theme?: string }) {
  ...
  return (
    <>
      ...
      {item.cta && (
        <CtaButton cta={item.cta} variant="ghost" style={{ marginTop: "var(--sp-m)" }} />
      )}
    </>
  );
}
